...
async fn mixed_success_and_dead_letter_batch() {
    let bus = Arc::new(InMemoryBus::new());
    let handler = Arc::new(FlakyHandler { succeed_at_attempt: 100 }); // always fails
    let policy = RetryPolicy { max_attempts: 2, ..RetryPolicy::default() };
    let orch = Orchestrator::new(bus.clone(), handler, policy, "jobs");

    orch.submit(&Job::new("doomed", "key-d", "{}")).await.unwrap();
    let outcomes = orch.run_until_idle().await.unwrap();

    assert!(outcomes.iter().any(|o| matches!(o, StepOutcome::DeadLettered(_))));
