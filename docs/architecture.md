...
```text
flux-core pure logic: job state machine, retry, idempotency, dead-letter (no I/O, no async)
flux-bus the EventBus trait + in-memory backend + optional Kafka backend flux-engine orchestration:
